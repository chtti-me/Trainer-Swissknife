...
#### 模組 B：課程規劃幫手（AI Agent 風格）
- 2 步驟流程：**貼上需求 → AI 一次產出**（取代舊版 7 步驟精靈）
- 需求文字輸入 / 檔案上傳（.txt / .pdf / .docx）
- 可選填偏好班名與偏好時數
- AI 一次產出開班計劃表核心欄位：建議班名、目標、對象、預備知識、課程模組（名稱 + 時數）、建議講師人選（姓名、教學領域、來源標示）
- AI 產出時併入 **AI 技能脈絡**（全院共用 + 登入者個人，見 `docs/AI_SKILLS_CONTEXT.md`）
- 講師搜尋策略：OpenAI 模式使用 Responses API + `web_search_preview` 搜尋實際講師；Gemini 模式由 AI 根據領域知識推薦，標註「AI 推薦（建議人工查證）」
- 草案匯出（Markdown / HTML / JSON）
- 版本保存
...
#### 工具 4：EDM／DM 產生器
- 路由：`/tools/edm-generator`（Next.js 頁面）；後端 `POST /api/tools/edm-generator`
- 貼上開班資訊文字、HTML 或 OCR（光學字元辨識）擷取內容，解析欄位後選模板與配色
- 拖曳排序圖片、產出宣傳用 HTML（電子郵件／網頁用）
- AI 文案模式會併入 **AI 技能脈絡**（同 `docs/AI_SKILLS_CONTEXT.md`）
...
- EDM 宣傳文案（`POST /api/tools/edm-generator`）
...
支援 OpenAI-compatible API，並可透過 `AI_PROVIDER` 切換：
...
│   │   │       └── edm-generator/   # EDM／DM 產生器
│   │   └── api/                     # REST API（含 admin、agent、planning、similarity 等）
│   ├── components/
│   └── lib/
│       ├── agent/                   # v3 Agent 核心引擎
...
│       ├── planning/                # 課程規劃幫手
│       └── ...                      # prisma、auth、ai-provider、ai-skills、similarity、edm、utils…
├── .env.example
├── docker-compose.yml
├── package.json
└── README.md
