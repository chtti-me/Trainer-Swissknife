...
- Node.js 22+（建議由 nvm 安裝，本專案附 `.nvmrc`）
- Supabase 專案（免費方案即可，含 pgvector 擴充）
- （選配）OpenAI 或 Gemini API Key
- 若於 Windows 開發：請使用 **WSL 2（Windows Subsystem for Linux 2，Windows 內的 Linux 子系統第 2 代）+ Ubuntu 22.04 / 24.04**，並把專案放到 Linux 檔案系統（`~/...`），不要放在 `/mnt/c/...`。原因見下方「跨環境執行」章節。

### 1. 安裝依賴

```bash
# 第一次設定（會自動安裝 nvm + .nvmrc 指定的 Node + npm install）
npm run setup
...
> `npm run setup` 對應 `bash scripts/setup-wsl.sh`，會以無 sudo 方式安裝 nvm（Node Version Manager，Node 版本管理器），按 `.nvmrc` 切到 Node 22 LTS（Long Term Support，長期支援版），再執行 `npm install`。
...
```bash
npm run dev      # 等同 npm run server
```

預設埠為 **3001**；要改埠：
- bash／WSL：`DEV_PORT=4000 npm run dev`
- PowerShell：`$env:DEV_PORT=4000; npm run dev`

啟動器會：

1. 自動載入 nvm 並依 `.nvmrc` 切換 Node 版本
2. 從 PATH 中移除 `/mnt/c/...` 的 Windows 版 Node，避免誤抓
3. 釋放佔用 `DEV_PORT` 的舊行程
4. 在 WSL 下自動啟用 Watchpack／Chokidar polling（解決 9P 共享路徑檔案監控失靈）
5. 直接執行 `next dev`（**不**透過 npm 與 cmd.exe，跨環境最穩）

若不需要任何前置處理，可用 `npm run dev:raw`（直接 `next dev`）。

開啟 http://localhost:3001

#### 跨環境執行（同一份專案，Windows 與 WSL 都能跑）

本專案實際存放在 WSL 的 Linux 檔案系統（`/home/<user>/...`）；Windows 看到的會是
UNC（Universal Naming Convention，網路共享路徑）`\\wsl.localhost\<distro>\...`。
若直接以 Windows 版 Node 啟動，會踩到三個經典雷區：

1. `cmd.exe`（命令提示字元）不支援 UNC 為 cwd（current working directory，目前
   工作目錄），會把 cwd 改回 `C:\Windows`，導致 `npm error Missing script: "dev"`。
2. Windows Node 監控 9P（WSL 的共享協定）上的檔案常常卡死、不出 `Ready`。
3. 在 WSL 內又可能誤抓 `/mnt/c/Program Files/nodejs/npm`，繞回 Windows 版。

因此 `npm run dev` 一律會在 **WSL Linux 內、用 Linux 版 Node** 執行。實際路徑：

| 你在哪裡執行                          | 啟動指令          | 實際發生什麼事                                                                            |
| ------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------- |
| WSL Ubuntu 終端                       | `npm run dev`     | `node scripts/dev.mjs` → 直接 `bash scripts/dev-wsl.sh`                                   |
| Windows PowerShell（cwd 為映射磁碟）  | `npm run dev`     | `dev.mjs` 偵測 Windows、用 `Get-PSDrive` / `net use` 找出 WSL UNC、轉成 Linux 路徑後送進 WSL |
| Windows 終端（cwd 為 UNC `\\wsl.localhost\…`） | `.\dev.cmd` | `dev.cmd` 從 `%~dp0` 直接解析 Linux 路徑、繞過 Windows 版 node/npm，直接 `wsl.exe` 進去跑   |

需要時可由環境變數覆寫：

- `WSL_DISTRO=Ubuntu-24.04`：指定要進入的 WSL 發行版（預設 `Ubuntu-24.04`）
- `WATCHPACK_NO_POLL=1`：關閉 polling 模式（預設於 WSL 內啟用）

第一次在新機器上執行：在 WSL Ubuntu 內 `cd` 到專案後跑 `npm run setup`，
之後不論 Windows 或 WSL 終端都用 `npm run dev` 即可。
