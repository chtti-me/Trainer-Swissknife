...
export type SupportedUploadExt = "txt" | "docx" | "pdf" | "xlsx" | "csv";

export function detectUploadExt(filename: string): SupportedUploadExt | null {
...
/**
 * 解析使用者上傳的需求檔案為純文字。
 * 支援 .txt / .docx / .pdf / .xlsx / .csv，最大 10MB。
 */
export async function parseUploadToText(params: {
  filename: string;
  mimeType?: string;
  buffer: Buffer;
}): Promise<{ text: string; ext: SupportedUploadExt }> {
